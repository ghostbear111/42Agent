...
>
> **版本：v1.1.0「星球探索」** · Unity 6000.3.11f1 · 命名空间 `GalaxyAgent`

## 本版概览

v1.1.0「创世」在 v1.0 完整可玩基础上，把**「创造」的权力交还给玩家**，主题有二：

1. **LLM 创世** —— 在地图生成场景用自然语言对话，让本地大模型为你"捏"一颗专属星球（命名 / 地形 / 资源 / 风险 / 天气 / 昼夜），并给出星球介绍。
2. **Mod 换图** —— 打包成 exe 后，玩家在游戏目录的 `Mods/` 文件夹丢入图片即可替换游戏内**全部图片元素**（头像 / 资源 / 天气 / 背景 / 面板皮肤 / 按钮图标），无需懂代码、无需重新打包。

并附带一系列体验优化：Loading 进度页、跨场景 LLM 配置隔离、图片拉伸变形修复。

> *v1.0 让文明启程，v1.1 让玩家造物。*
---
